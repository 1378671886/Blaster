...
void ABlasterPlayerController_Player::SetHUDAnnouncementCountdown(float CountdownTime)
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD &&
...
void ABlasterPlayerController_Player::Tick(float DeltaTime)
{
	...
	if (PlayerState)
	{
		float Ping = PlayerState->GetCompressedPing() * 4;
		SetHUDPing(Ping);
	}

}

void ABlasterPlayerController_Player::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ABlasterPlayerController_Player, MatchState);
}
...
void ABlasterPlayerController_Player::PollInit()
{
...
void ABlasterPlayerController_Player::OnMatchStateSet(FName State)
{
	MatchState = State;
...
void ABlasterPlayerController_Player::ServerReportPingStatus_Implementation(bool bHighPing)
{
...
void ABlasterPlayerController_Player::SetHUDPing(float Ping)
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	...
	if (bHUDValid)
	{
		FString PingString = FString::Printf(TEXT("%d ms"), FMath::RoundToInt(Ping));
...
void ABlasterPlayerController_Player::HighPingWarning()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
...
void ABlasterPlayerController_Player::StopHighPingWarning()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
...
void ABlasterPlayerController_Player::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime >= CheckPingFrequency)
	{
		...
		if (PlayerState)
		{
			if (PlayerState->GetCompressedPing() * 4 > HighPingThreshold) //ping被压缩四倍 所以乘四
			{
				HighPingWarning();
...
void ABlasterPlayerController_Player::ClientJoinMidgame_Implementation(FName StateOfMatch, float Warmup, float Match, float Cooldown, float StartingTime, bool bIsTeamsMatch)
{
	WarmupTime = Warmup;
	MatchTime = Match;
...
void ABlasterPlayerController_Player::ServerCheckMatchState_Implementation()
{
